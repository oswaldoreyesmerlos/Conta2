...
FUNCTION ArticulosForm( lNuevo, cCodigo )
    ...
    oWin:AddCtrl( TLabel():New(  2, 72, "Es servicio   :", oWin ) )
    oWin:AddCtrl( TLabel():New(  4, 72, "Cta. Venta   :", oWin ) )
    oWin:AddCtrl( TLabel():New(  6, 72, "Cta. Compra  :", oWin ) )
    oWin:AddCtrl( TLabel():New(  8, 72, "Costo         :", oWin ) )
    oWin:AddCtrl( TLabel():New( 10, 72, "Precio        :", oWin ) )
    oWin:AddCtrl( TLabel():New( 12, 72, "IVA %         :", oWin ) )
    oWin:AddCtrl( TLabel():New( 14, 72, "Tipo IVA      :", oWin ) )
    oWin:AddCtrl( TLabel():New( 16, 72, "Dto. %        :", oWin ) )
    ...
    oChkSer := TCheck():New(  2, 90, "Es servicio", lEsServ, oWin )
    oGCtaV  := TGet():New(  4, 90, cCtaVta, "@!",        oWin )
    oGCtaC  := TGet():New(  6, 90, cCtaCom, "@!",        oWin )
    oGCost  := TGet():New(  8, 90, nCosto,  "999,999.99", oWin )
    oGPre   := TGet():New( 10, 90, nPrecio, "999,999.99", oWin )
    oGIva   := TGet():New( 12, 90, nIva,    "99.99",      oWin )
    oGTip   := TGet():New( 14, 90, cTipoIva,"@!",         oWin )
    oGDesc  := TGet():New( 16, 90, nDescuent,"99.99",      oWin )

    oChkBaj := TCheck():New( 18, 72, "Baja", lBaja, oWin )
